			...
			<div class="col-md-4 form-group">
				...
				<select name="mode_perhitungan" id="mode_perhitungan" class="form-control" required>
					<option value="otomatis" <?php echo ($modePerhitungan === 'otomatis') ? 'selected' : ''; ?>>Hitung
						Otomatis</option>
					<option value="manual" <?php echo ($modePerhitungan === 'manual') ? 'selected' : ''; ?>>Input Zakat
						Manual</option>
				</select>
...
<script>
	(function () {
		...
		const modePerhitunganEl = document.getElementById('mode_perhitungan');
		...
		function recalc() {
			const hartaBersih = hitungHartaBersih();
		...
		function updateModePerhitungan() {
			const mode = modePerhitunganEl ? modePerhitunganEl.value : 'otomatis';
			if (!totalZakatEl) return;

			if (mode === 'manual') {
				totalZakatEl.readOnly = false;
				hitungHartaBersih();
			} else {
				totalZakatEl.readOnly = true;
				recalc();
			}
		}
		...
		if (modePerhitunganEl) {
			modePerhitunganEl.addEventListener('change', updateModePerhitungan);
		}

		updateModePerhitungan();
	})();
</script>